...
> **Accuracy note (verified against `Modules/Seo/app` 2026-09):** the real DTO
> namespace is `Modules\Seo\Datas\SocialShareData` (the code sample below still
> says `Modules\Seo\Data`, the pre-refactor namespace). The real widget is the
> simpler `SocialShareWidget extends XotBaseSchemaWidget` described in
> [social-share-component.md](./social-share-component.md)'s accuracy note, not
> the Alpine.js/icon-component design sketched here. `GenerateSocialShareLinksAction`
> and its URL-pattern table do match the real action.
...
- **DTO**: [SocialShareData.php](../app/Datas/SocialShareData.php)
- **Action**: [GenerateSocialShareLinksAction.php](../app/Actions/GenerateSocialShareLinksAction.php)
- **Widget**: [SocialShareWidget.php](../app/Filament/Widgets/SocialShareWidget.php)

(The links above previously pointed at absolute paths under a different
project, `base_laravelpizza`, evidently copied over along with this doc's
content — fixed to relative paths within this repo.)
