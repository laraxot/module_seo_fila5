...
> **Accuracy note (verified against `Modules/Seo/app` 2026-09):** the real, shipped
> `SocialShareWidget` extends `XotBaseSchemaWidget` (not `XotBaseWidget`), has no
> `mount()`/`getShareUrls()`/`enabledServices`/`getOpenGraphData()` methods, and
> renders `getViewData()` by calling `GenerateSocialShareLinksAction::execute()`
> with a `Modules\Seo\Datas\SocialShareData` DTO. Its view is
> `seo::filament.widgets.social-share`
> (`Modules/Seo/resources/views/filament/widgets/social-share.blade.php`), not
> `seo::widgets.social-share`. The "Implementazione Completa - Widget Avanzato"
> section below is an aspirational design, not the current implementation —
> treat it as a proposal, not a reference. `GenerateSocialShareLinksAction`'s
> URL patterns (Facebook/Twitter/LinkedIn/WhatsApp/Telegram) do match the real
> `app/Actions/GenerateSocialShareLinksAction.php`.
