---
title: "Seo Module - PHPStan Type Compliance"
type: concept
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Seo \
  --level=max \
  --no-progress
```
...
```bash
vendor/bin/phpstan analyse laravel/Modules/Seo --level=max
```
...
# Verbose mode
vendor/bin/phpstan analyse laravel/Modules/Seo --level=max -v
```
...
```bash
vendor/bin/pest laravel/Modules/Seo/tests --parallel
```
