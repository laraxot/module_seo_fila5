...
> **Accuracy note (verified against `Modules/Seo/app`/`tests` 2026-09):** there is
> no `Modules\Seo\Models\SeoMetadata` model, no `database.connections.seo`
> connection, and no `/api/seo/metadata/{slug}` route in this codebase — the
> "Basic Seo Test", "Configuration Test", "Database Issues", and "API Tests"
> examples below are illustrative/aspirational, not runnable against the
> current module. The real test suite covers the `Metatag` facade stack:
> `tests/Feature/MetatagFacadeAdapterTest.php`,
> `tests/Unit/Adapters/MetatagFacadeAdapterExtendedTest.php`,
> `tests/Unit/Actions/Metatag/ReplaceMetatagDataActionTest.php`,
> `tests/Unit/Datas/MetatagDataTest.php`, `tests/Unit/Facades/MetatagFacadeTest.php`,
> plus the legacy `tests/Feature/MetatagServiceTest.php` /
> `tests/Unit/Services/MetatagServiceExtendedTest.php` against the deprecated
> `MetatagService`. Run the real suite with `./vendor/bin/pest Modules/Seo/tests`.
